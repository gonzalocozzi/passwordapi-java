pipeline {
    ...
    stages {
        ...
        stage('image') {
            steps {
                unstash 'binaries'
                sh "docker build -t gonzalocozzi/passwordapi-java:taller\${BUILD_ID} . --build-arg JAR_FILE=./target/passwordapi-1.5.2.jar"
            }
        }
        stage('publish') {
            steps {
                sh "docker push gonzalocozzi/passwordapi-java:taller\${BUILD_ID}"
            }
        }
        stage('deploy') {
            steps {
